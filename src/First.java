
//ДЗ_1
public class First {
    public static void main (String[] args) {
        System.out.println("                  РУБАИ");
        System.out.println("Много лет размышлял я над жизнью земной.");
        System.out.println("Непонятного нет для меня под луной.");
        System.out.println("Мне известно, что мне ничего не известно!");
        System.out.println("Вот последняя вправда открытая мной.");
        System.out.println("                                 О.Кайям.");

//От Сергея (переменные)
        String x = "IRA";
        String y = "vova";
        String love = "vb ngfngfnmgfnhg";
//        System.out.println(x + " " + " + " + x + " = " + love );

    }
}